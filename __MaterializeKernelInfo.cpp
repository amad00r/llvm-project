...
struct LogicalSignatureInference {
  ...
  ExtendedType *rec(const Value *V) {
    ...
    for (const Use &U : V->uses()) {
      ...
      if (Ty) {
        ...
        LLVM_DEBUG(
            ...
            dbgs() << " has an estimated type <" << Ty->dump()
                   << "> in instruction { " << *U.getUser() << " }\n");
      }
    }
    ...
    if (ReinterpretSet.empty()) {
      ...
      ValueTypeMap[V] = Ty;
      LLVM_DEBUG(V->printAsOperand(dbgs(), false);
                 dbgs() << " falls back to implementation type <" << Ty->dump()
                        << "> as it does not have available uses\n");
      return Ty;
    }

    for (size_t i = 1; i < ReinterpretSet.size(); ++i)
      if (!ExtendedType::isStructurallyEqual(ReinterpretSet[0],
                                             ReinterpretSet[i])) {
        ExtendedType *Ty = getExtType(V->getType());
        ValueTypeMap[V] = Ty;
        LLVM_DEBUG(V->printAsOperand(dbgs(), false);
                   dbgs() << " falls back to implementation type <"
                          << Ty->dump() << "> to be conservative\n");
        return Ty;
      }
...
KernelArgInfo getKernelArgInfo(Type *Ty, int Indirection = 0) {
  assert(Ty);
  if (Ty->isIntegerTy())
    return KernelArgInfo::getIntegerTy(cast<IntegerType>(Ty)->getBitWidth(),
                                       Indirection);
  if (Ty->isFloatTy())
    return KernelArgInfo::getFloatTy(Indirection);
  if (Ty->isDoubleTy())
    return KernelArgInfo::getDoubleTy(Indirection);
  if (Ty->isPointerTy())
    return KernelArgInfo::getOpaquePointerTy(Indirection);
  return KernelArgInfo::getUnknownTy();
}

KernelArgInfo getKernelArgInfo(LogicalSignatureInference::ExtendedType *ExtTy) {
  uint32_t Indirection = 0;
  while (ExtTy->isTransparentPointerTy()) {
    ++Indirection;
    ExtTy = ExtTy->unwrap();
  }
  return getKernelArgInfo(ExtTy->getLLVMType(), Indirection);
}
...
bool materializeKernelInfo(Function &F) {
  ...
  if (omp::isOpenMPKernel(F)) {
    ...
    for (LogicalSignatureInference::ExtendedType *ArgType : LSI(F))
      EncodedKernelInfo.emplace_back(getKernelArgInfo(ArgType).getEncodedLE());
  } else {
    for (const Argument &Arg : F.args())
